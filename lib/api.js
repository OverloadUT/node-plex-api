    ...
    request(reqOpts, function onResponse(err, response, body) {
        ...
        if (response.statusCode === 401) {
            if (self.authenticator === undefined) {
                var authenticationError = new Error('Plex Server denied request, you must provide a way to authenticate! ' +
                ...
                return deferred.reject(authenticationError);
            }
        ...
        if (response.statusCode < 200 || response.statusCode > 299) {
            var httpStatusCodeError = new Error('Plex Server didnt respond with a valid 2xx status code, response code: ' + response.statusCode);
            httpStatusCodeError.name = 'HttpStatusCodeError';
            httpStatusCodeError.statusCode = response.statusCode;
            return deferred.reject(httpStatusCodeError);
        }
...
PlexAPI.prototype._authenticate = function _authenticate() {
    ...
    if (this.authToken) {
        var authenticationError = new Error('Permission denied even after attempted authentication :( Wrong username and/or password maybe?');
        authenticationError.name = 'AuthenticationError';
        return deferred.reject(authenticationError);
    }
